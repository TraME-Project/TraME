...
subroutine dqage ( f, f_args, a, b, epsabs, epsrel, key, limit, result, abserr, &
...
!
  ...
  real ( kind = 8 ) a,abserr,alist,area,area1,area12,area2,a1,a2,b, &
    ...
    epsabs,epsrel,errbnd,errmax,error1,error2,erro12,errsum,f,f_args, &
...
!
  ...
  if(key.ge.7) keyf = 6
  neval = 0
  if(keyf.eq.1) call dqk15(f,f_args,a,b,result,abserr,defabs,resabs)
  if(keyf.eq.2) call dqk21(f,f_args,a,b,result,abserr,defabs,resabs)
  if(keyf.eq.3) call dqk31(f,f_args,a,b,result,abserr,defabs,resabs)
  if(keyf.eq.4) call dqk41(f,f_args,a,b,result,abserr,defabs,resabs)
  if(keyf.eq.5) call dqk51(f,f_args,a,b,result,abserr,defabs,resabs)
  if(keyf.eq.6) call dqk61(f,f_args,a,b,result,abserr,defabs,resabs)
  last = 1
  rlist(1) = result
  elist(1) = abserr
  iord(1) = 1
...
!
    ...
    if(keyf.eq.1) call dqk15(f,f_args,a1,b1,area1,error1,resabs,defab1)
    if(keyf.eq.2) call dqk21(f,f_args,a1,b1,area1,error1,resabs,defab1)
    if(keyf.eq.3) call dqk31(f,f_args,a1,b1,area1,error1,resabs,defab1)
    if(keyf.eq.4) call dqk41(f,f_args,a1,b1,area1,error1,resabs,defab1)
    if(keyf.eq.5) call dqk51(f,f_args,a1,b1,area1,error1,resabs,defab1)
    if(keyf.eq.6) call dqk61(f,f_args,a1,b1,area1,error1,resabs,defab1)

    if(keyf.eq.1) call dqk15(f,f_args,a2,b2,area2,error2,resabs,defab2)
    if(keyf.eq.2) call dqk21(f,f_args,a2,b2,area2,error2,resabs,defab2)
    if(keyf.eq.3) call dqk31(f,f_args,a2,b2,area2,error2,resabs,defab2)
    if(keyf.eq.4) call dqk41(f,f_args,a2,b2,area2,error2,resabs,defab2)
    if(keyf.eq.5) call dqk51(f,f_args,a2,b2,area2,error2,resabs,defab2)
    if(keyf.eq.6) call dqk61(f,f_args,a2,b2,area2,error2,resabs,defab2)
!
...
subroutine dqag ( f, f_args, a, b, epsabs, epsrel, key, result, abserr, neval, ier, &
...
!
  ...
  call dqage(f,f_args,a,b,epsabs,epsrel,key,limit,result,abserr,neval, &
...
subroutine dqagie ( f, f_args, bound, inf, epsabs, epsrel, limit, result, abserr, &
  neval, ier, alist, blist, rlist, elist, iord, last )
...
!
  ...
  real ( kind = 8 ) abseps,abserr,alist,area,area1,area12,area2,a1, &
    ...
    errbnd,errmax,error1,error2,erro12,errsum,ertest,f,f_args,oflow,resabs, &
...
!
  ...
  call dqk15i(f,f_args,boun,inf,0.0D+00,0.1D+01,result,abserr, &
    defabs,resabs)
!
!  test on accuracy
!
...
!
    ...
    b2 = blist(maxerr)
    erlast = errmax
    call dqk15i(f,f_args,boun,inf,a1,b1,area1,error1,resabs,defab1)
    call dqk15i(f,f_args,boun,inf,a2,b2,area2,error2,resabs,defab2)
!
!  improve previous approximations to integral
!  and error and test for accuracy.
!
...
subroutine dqagi ( f, f_args, bound, inf, epsabs, epsrel, result, abserr, neval, &
...
!
  ...
  real ( kind = 8 ) abserr,bound,epsabs,epsrel,f,f_args,result,work
...
!
  ...
  call dqagie(f,f_args,bound,inf,epsabs,epsrel,limit,result,abserr, &
...
subroutine dqagpe(f,f_args,a,b,npts2,points,epsabs,epsrel,limit,result, &
...
!
  ...
  real ( kind = 8 ) a,abseps,abserr,alist,area,area1,area12,area2,a1, &
    ...
    errmax,error1,erro12,error2,errsum,ertest,f,f_args,oflow,points,pts, &
...
!
  ...
  do i = 1,nint
    b1 = pts(i+1)
    call dqk21(f,f_args,a1,b1,area1,error1,defabs,resa)
...
!
    ...
    b2 = blist(maxerr)
    erlast = errmax
    call dqk21(f,f_args,a1,b1,area1,error1,resa,defab1)
    call dqk21(f,f_args,a2,b2,area2,error2,resa,defab2)
!
!  improve previous approximations to integral
!  and error and test for accuracy.
!
...
subroutine dqagp ( f, f_args, a, b, npts2, points, epsabs, epsrel, result, abserr, &
...
!
  ...
  real ( kind = 8 ) a,abserr,b,epsabs,epsrel,f,f_args,points,result,work
...
!
  ...
  call dqagpe(f,f_args,a,b,npts2,points,epsabs,epsrel,limit,result,abserr, &
...
subroutine dqagse(f,f_args,a,b,epsabs,epsrel,limit,result,abserr,neval, &
...
!
  ...
  real ( kind = 8 ) a,abseps,abserr,alist,area,area1,area12,area2,a1, &
    ...
    error1,error2,erro12,errsum,ertest,f,f_args,oflow,resabs,reseps,result, &
...
!
  ...
  oflow = huge ( oflow )
  ierro = 0
  call dqk21(f,f_args,a,b,result,abserr,defabs,resabs)
!
!  test on accuracy.
!
  dres =  abs ( result)
...
!
    ...
    b2 = blist(maxerr)
    erlast = errmax
    call dqk21(f,f_args,a1,b1,area1,error1,resabs,defab1)
    call dqk21(f,f_args,a2,b2,area2,error2,resabs,defab2)
!
!  improve previous approximations to integral
!  and error and test for accuracy.
!
...
subroutine dqags ( f, f_args, a, b, epsabs, epsrel, result, abserr, neval, ier, &
  limit, lenw, last, iwork, work )
...
!
  ...
  real ( kind = 8 ) a,abserr,b,epsabs,epsrel,f,f_args,result,work
...
!
  ...
  call dqagse(f,f_args,a,b,epsabs,epsrel,limit,result,abserr,neval, &
    ier,work(1),work(l1),work(l2),work(l3),iwork,last)
!
!  call error handler if necessary.
!
...
subroutine dqawce(f,f_args,a,b,c,epsabs,epsrel,limit,result,abserr,neval, &
  ier,alist,blist,rlist,elist,iord,last)
...
!
  ...
  real ( kind = 8 ) a,aa,abserr,alist,area,area1,area12,area2,a1,a2, &
    ...
    errbnd,errmax,error1,erro12,error2,errsum,f,f_args,result,rlist,uflow
...
!
  ...
  ier=0
  krule = 1
  call dqc25c(f,f_args,aa,bb,c,result,abserr,krule,neval)
  last = 1
  rlist(1) = result
  elist(1) = abserr
  iord(1) = 1
...
!
    ...
    a2 = b1
    krule = 2
    call dqc25c(f,f_args,a1,b1,c,area1,error1,krule,nev)
    neval = neval+nev
    call dqc25c(f,f_args,a2,b2,c,area2,error2,krule,nev)
    neval = neval+nev
!
!  improve previous approximations to integral
!  and error and test for accuracy.
...
subroutine dqawc ( f, f_args, a, b, c, epsabs, epsrel, result, abserr, neval, ier, &
  limit, lenw, last, iwork, work )
...
!
  ...
  real ( kind = 8 ) a,abserr,b,c,epsabs,epsrel,f,f_args,result,work
  integer ( kind = 4 ) ier,iwork,last,lenw,limit,lvl,l1,l2,l3,neval
  dimension iwork(limit),work(lenw)
...
!
  ...
  l2 = limit+l1
  l3 = limit+l2
  call dqawce(f,f_args,a,b,c,epsabs,epsrel,limit,result,abserr,neval,ier, &
...
subroutine dqawfe(f,f_args,a,omega,integr,epsabs,limlst,limit,maxp1, &
...
!
  ...
  real ( kind = 8 ) a,abseps,abserr,alist,blist,chebmo,correc,cycle, &
    ...
    epsabs,errsum,f,f_args,fact,omega,p,pi,p1,psum,reseps,result,res3la, &
    rlist,rslst,uflow
...
!
  if(integr.eq.1) then
    call dqagie(f,f_args,0.0D+00,1,epsabs,0.0D+00,limit, &
...
!
    dla = lst
    epsa = eps*fact
    call dqawoe(f,f_args,c1,c2,omega,integr,epsa,0.0D+00,limit,lst,maxp1, &
...
subroutine dqawf ( f, f_args, a, omega, integr, epsabs, result, abserr, neval, ier, &
...
!
  ...
  l6 = limit+l5
  ll2 = limit+l1
  call dqawfe(f,f_args,a,omega,integr,epsabs,limlst,limit,maxp1,result, &
...
subroutine dqawoe ( f, f_args, a, b, omega, integr, epsabs, epsrel, limit, icall, &
...
!
  ...
  real ( kind = 8 ) a,abseps,abserr,alist,area,area1,area12,area2,a1, &
    ...
    errbnd,errmax,error1,erro12,error2,errsum,ertest,f,f_args,oflow, &
...
!
  ...
  momcom = 0
    5 call dqc25f(f,f_args,a,b,domega,integr,nrmom,maxp1,0,result,abserr, &
...
!
    ...
    b2 = blist(maxerr)
    erlast = errmax
    call dqc25f(f,f_args,a1,b1,domega,integr,nrmom,maxp1,0, &
    area1,error1,nev,resabs,defab1,momcom,chebmo)
    neval = neval+nev
    call dqc25f(f,f_args,a2,b2,domega,integr,nrmom,maxp1,1, &
...
subroutine dqawo ( f, f_args, a, b, omega, integr, epsabs, epsrel, result, abserr, &
...
!
  ...
  real ( kind = 8 ) a,abserr,b,epsabs,epsrel,f,f_args,omega,result,work
...
!
  ...
  l3 = limit+l2
  l4 = limit+l3
  call dqawoe(f,f_args,a,b,omega,integr,epsabs,epsrel,limit,1,maxp1,result, &
...
subroutine dqawse(f,f_args,a,b,alfa,beta,integr,epsabs,epsrel,limit, &
     result,abserr,neval,ier,alist,blist,rlist,elist,iord,last)
...
!
  ...
  real ( kind = 8 ) a,abserr,alfa,alist,area,area1,area12,area2,a1, &
    ...
    epsabs,epsrel,errbnd,errmax,error1,erro12,error2,errsum,f,f_args, &
...
!
  centre = 0.5D+00*(b+a)
  call dqc25s(f,f_args,a,b,a,centre,alfa,beta,ri,rj,rg,rh,area1, &
    error1,resas1,integr,nev)
  neval = nev
  call dqc25s(f,f_args,a,b,centre,b,alfa,beta,ri,rj,rg,rh,area2, &
...
!
    ...
    call dqc25s(f,f_args,a,b,a1,b1,alfa,beta,ri,rj,rg,rh,area1, &
    error1,resas1,integr,nev)
    neval = neval+nev
    call dqc25s(f,f_args,a,b,a2,b2,alfa,beta,ri,rj,rg,rh,area2, &
    error2,resas2,integr,nev)
    neval = neval+nev
!
...
subroutine dqaws ( f, f_args, a, b, alfa, beta, integr, epsabs, epsrel, result, &
...
!
  ...
  real ( kind = 8 ) a,abserr,alfa,b,beta,epsabs,epsrel,f,f_args,result,work
...
!
  ...
  call dqawse(f,f_args,a,b,alfa,beta,integr,epsabs,epsrel,limit,result, &
...
subroutine dqc25c(f,f_args,a,b,c,result,abserr,krul,neval)
...
!
  ...
  real ( kind = 8 ) a,abserr,ak22,amom0,amom1,amom2,b,c,cc,centr, &
    cheb12,cheb24,dqwgtc,f,f_args,fval,hlgth,p2,p3,p4,resabs, &
...
!
  krul = krul-1
  call dqk15w(f,f_args,dqwgtc,c,p2,p3,p4,kp,a,b,result,abserr, &
    resabs,resasc)
  neval = 15
...
!
  ...
  centr = 0.5D+00*(b+a)
  neval = 25
  fval(1) = 0.5D+00*f(hlgth+centr, f_args)
  fval(13) = f(centr, f_args)
  fval(25) = 0.5D+00*f(centr-hlgth, f_args)

  do i=2,12
    u = hlgth*x(i-1)
    isym = 26-i
    fval(i) = f(u+centr, f_args)
    fval(isym) = f(centr-u, f_args)
  end do
!
...
subroutine dqc25f(f,f_args,a,b,omega,integr,nrmom,maxp1,ksave,result, &
...
!
  ...
  real ( kind = 8 ) a,abserr,ac,an,an2,as,asap,ass,b,centr,chebmo, &
    ...
    d2,estc,ests,f,f_args,fval,hlgth,oflow,omega,parint,par2,par22, &
...
!
  ...
  call dqk15w(f,f_args,dqwgtf,omega,p2,p3,p4,integr,a,b,result, &
...
!
  fval(1) = 0.5D+00*f(centr+hlgth, f_args)
  fval(13) = f(centr, f_args)
  fval(25) = 0.5D+00*f(centr-hlgth, f_args)
  do i = 2,12
    isym = 26-i
    fval(i) = f(hlgth*x(i-1)+centr, f_args)
    fval(isym) = f(centr-hlgth*x(i-1), f_args)
  end do
  call dqcheb(x,fval,cheb12,cheb24)
!
...
subroutine dqc25s(f,f_args,a,b,bl,br,alfa,beta,ri,rj,rg,rh,result, &
...
!
  ...
  real ( kind = 8 ) a,abserr,alfa,b,beta,bl,br,centr,cheb12,cheb24, &
    dc,f,f_args,factor,fix,fval,hlgth,resabs,resasc,result,res12, &
...
!
!
  call dqk15w(f,f_args,dqwgts,a,b,alfa,beta,integr,bl,br, &
...
!
  ...
  centr = 0.5D+00*(br+bl)
  fix = b-centr
  fval(1) = 0.5D+00*f(hlgth+centr, f_args)*(fix-hlgth)**beta
  fval(13) = f(centr, f_args)*(fix**beta)
  fval(25) = 0.5D+00*f(centr-hlgth, f_args)*(fix+hlgth)**beta
  do i=2,12
    u = hlgth*x(i-1)
    isym = 26-i
    fval(i) = f(u+centr, f_args)*(fix-u)**beta
    fval(isym) = f(centr-u, f_args)*(fix+u)**beta
  end do
...
!
  ...
  centr = 0.5D+00*(br+bl)
  fix = centr-a
  fval(1) = 0.5D+00*f(hlgth+centr, f_args)*(fix+hlgth)**alfa
  fval(13) = f(centr, f_args)*(fix**alfa)
  fval(25) = 0.5D+00*f(centr-hlgth, f_args)*(fix-hlgth)**alfa
  do i=2,12
    u = hlgth*x(i-1)
    isym = 26-i
    fval(i) = f(u+centr, f_args)*(fix+u)**alfa
    fval(isym) = f(centr-u, f_args)*(fix-u)**alfa
  end do
...
subroutine dqk15(f,f_args,a,b,result,abserr,resabs,resasc)
...
!
  ...
  real ( kind = 8 ) a,absc,abserr,b,centr,dhlgth, &
    epmach,f,f_args,fc,fsum,fval1,fval2,fv1,fv2,hlgth,resabs,resasc, &
...
!
  ...
  do j=1,3
    jtw = j*2
    absc = hlgth*xgk(jtw)
    fval1 = f(centr-absc, f_args)
    fval2 = f(centr+absc, f_args)
    fv1(jtw) = fval1
    fv2(jtw) = fval2
    fsum = fval1+fval2
    resg = resg+wg(j)*fsum
...
subroutine dqk15i(f,f_args,boun,inf,a,b,result,abserr,resabs,resasc)
...
!
  ...
  real ( kind = 8 ) a,absc,absc1,absc2,abserr,b,boun,centr,dinf, &
    epmach,f,f_args,fc,fsum,fval1,fval2,fv1,fv2,hlgth, &
...
subroutine dqk15w(f,f_args,w,p1,p2,p3,p4,kp,a,b,result,abserr, resabs,resasc)
...
!
  fc = f(centr, f_args)*w(centr,p1,p2,p3,p4,kp)
  resg = wg(4)*fc
  resk = wgk(8)*fc
  resabs =  abs ( resk)
  ...
  do j=1,3
    ...
    absc1 = centr-absc
    absc2 = centr+absc
    fval1 = f(absc1, f_args)*w(absc1,p1,p2,p3,p4,kp)
    fval2 = f(absc2, f_args)*w(absc2,p1,p2,p3,p4,kp)
    fv1(jtw) = fval1
    fv2(jtw) = fval2
    fsum = fval1+fval2
    resg = resg+wg(j)*fsum
...
subroutine dqk21(f,f_args,a,b,result,abserr,resabs,resasc)
...
subroutine dqk31(f,f_args,a,b,result,abserr,resabs,resasc)
...
subroutine dqk41 ( f, f_args, a, b, result, abserr, resabs, resasc )
...
subroutine dqk51(f,f_args,a,b,result,abserr,resabs,resasc)
...
subroutine dqk61(f,f_args,a,b,result,abserr,resabs,resasc)
...
!
  ...
  do j=1,15
    jtw = j*2
    dabsc = hlgth*xgk(jtw)
    fval1 = f(centr-dabsc, f_args)
    fval2 = f(centr+dabsc, f_args)
    fv1(jtw) = fval1
    fv2(jtw) = fval2
    fsum = fval1+fval2
    resg = resg+wg(j)*fsum
...
subroutine dqng ( f, f_args, a, b, epsabs, epsrel, result, abserr, neval, ier )
...
!
  ...
  real ( kind = 8 ) a,absc,abserr,b,centr,dhlgth, &
    epmach,epsabs,epsrel,f,f_args,fcentr,fval,fval1,fval2,fv1,fv2, &
...
25  res43 = w43b(12)*fcentr
    ...
    do k=1,11
      ipx = ipx+1
      absc = hlgth*x3(k)
      fval = f(absc+centr, f_args)+f(centr-absc, f_args)
