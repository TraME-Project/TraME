...
trame::arums::rusc::rusc(const uint_t nbX_inp, const uint_t nbY_inp)
{
...
void
trame::arums::rusc::build(const uint_t nbX_inp, const uint_t nbY_inp)
{
    nbX = nbX_inp;
    nbY = nbY_inp;
}
...
trame::arums::rusc::build(const arma::mat& zeta_inp, const bool outside_option_inp)
{
    if (!outside_option_inp) {
        printf("TraME: rusc_build not defined when outside_option = false.\n");
        return;
    }
...
double
trame::arums::rusc::Gx(const arma::mat& U_x_inp, arma::mat& mu_x_out, const uint_t x)
const
{
    uint_t nbAlt = nbY + 1;
    int j,y,z;
...
double
trame::arums::rusc::Gstarx(const arma::mat& mu_x_inp, arma::mat &U_x_out, const uint_t x)
const
{
    arma::mat A_mu = arma::vectorise(aux_A.slice(x) * mu_x_inp); 

    U_x_out = A_mu + aux_b.row(x).t();
    double val_x = arma::accu(mu_x_inp % A_mu)/2 + arma::accu(mu_x_inp % aux_b.row(x).t()) + aux_c(x);
...
double
trame::arums::rusc::Gbarx(const arma::vec& Ubar_x, const arma::vec& mubar_x, arma::mat& U_x_out, arma::mat& mu_x_out, const uint_t x)
const
{
    uint_t nbAlt = nbY + 1;
    double val_x = 0.0;
...
trame::arums::empirical
trame::arums::rusc::simul(const uint_t n_draws, const uint_t seed)
const
{
    empirical emp_obj;
...
void
trame::arums::rusc::simul(empirical& obj_out, const uint_t n_draws, const uint_t seed)
const
{
...
void
trame::arums::rusc::simul_int(empirical& obj_out, const uint_t* n_draws_inp, const uint_t* seed_val)
const
{
    uint_t n_draws = 0;
